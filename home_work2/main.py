...
# ВАШ КОД:
pilot_mission_dict = {}
for pilot_name, pilot_data in data.items():
    pilot_mission_dict[pilot_name] = len(pilot_data['missions'])
...
print(
    f"Пилоты в порядке убывания количества миссий: {dict(sorted(pilot_mission_dict.items(), key=lambda item: item[1], reverse=True))}")
...
for pilot, missions in data.items():
    for mission in missions['missions']:
        drones_set.add(mission['drone'])

# вывод результата (допишите код)
print(f'Полеты совершались на дронах следующих моделей: {", ".join(drones_set)}')
...
for pilot, pilot_data in data.items():
    for missions in pilot_data['missions']:
        drone_name = missions['drone']
        if drone_name in drone_missions_dict:
            drone_missions_dict[drone_name] += 1
        else:
            drone_missions_dict[drone_name] = 1

# вывод результата (допишите код)
for drone, missions_count in drone_missions_dict.items():
    print(f'Дрон {drone} отлетал {missions_count} миссий')
...
for pilot, pilot_data in data.items():
    for mission in pilot_data['missions']:
        ...
        if drone in drone_to_multirotor:
            drone_to_multirotor[drone].missions.append(mission)
        else:
            drone_to_multirotor[drone] = MultirotorUAV(catalog_info['weight'], drone, catalog_info['brand'])
            drone_to_multirotor[drone].missions.append(mission)

miltirotors = [ x for _, x in drone_to_multirotor.items() ]
...
for multirotor in miltirotors:
    if multirotor.get_model().lower() == user_input:
        print(multirotor.get_info())
